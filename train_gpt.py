...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: None, block_mask: BlockMask, attn_cache):
        ...
        if ve is not None:
            ...
            #q = self.lambdas[2] * q + self.lambdas[3] * qe.view_as(q) # @KoszarskyB & @Grad62304977
...
class Block(nn.Module):
    ...
    def forward(self, x: Tensor, ve: None, x0: Tensor, block_mask: BlockMask, attn_cache):
        x = self.lambdas[0] * x + self.lambdas[1] * x0
        if self.attn is not None:
            x = x + self.attn(norm(x), ve, block_mask, attn_cache)
        x = x + self.mlp(norm(x))
        return x
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        #self.query_embeds = nn.ModuleList([nn.Embedding(vocab_size, model_dim) for _ in range(3)])
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        #qe = [query_embed(input_seq) for query_embed in self.query_embeds]
        # 012 ... 012 structure on token value embeddings by @YouJiacheng, improved on @leloykun's U-net structure
        ve = [ve[0], ve[1], ve[2]] + [None] * (len(self.blocks) - 6) + [ve[0], ve[1], ve[2]]
        #qe = [qe[0], qe[1], qe[2]] + [None] * (len(self.blocks) - 6) + [qe[0], qe[1], qe[2]]
        ...
        for i in range(len(self.blocks)):
            ...
            x = self.blocks[i](x, ve[i], x0, block_masks[i], attn_cache)
            #if i < n:
...
model: nn.Module = GPT(vocab_size=50257, num_layers=12, num_heads=6, model_dim=768,
                       max_seq_len=max(args.seq_len, args.val_seq_len)).cuda()
