...
class GPUMonitor:
    ...
    def start(self):
        ...
        self.thread.start()
        print("GPU monitoring started. Run your training in another cell.")
    ...
    def _monitor_gpu(self):
        ...
        while self.running:
            ...
            for gpu in gpus:
                ...
                self.gpu_mems.append(gpu.memoryUsed / gpu.memoryTotal * 100)

            # Plot every few iterations to reduce overhead
    ...
    def _plot_usage(self):
        ...
        # GPU Utilization
        ax[0].plot(self.times, self.gpu_utils, 'b-')
        ax[0].set_title('A100 GPU Performance')
        ...
        # Current utilization
        if self.gpu_utils:
            ax[0].text(0.02, 0.95, f"Current: {self.gpu_utils[-1]:.1f}%",
                       transform=ax[0].transAxes, fontweight='bold')

        # Memory Usage
        ax[1].plot(self.times, self.gpu_mems, 'r-')
        ...
        # Current memory
        if self.gpu_mems:
            ax[1].text(0.02, 0.95, f"Current: {self.gpu_mems[-1]:.1f}%",
    ...
    def get_stats(self):
        ...
        return {
            "avg_utilization": np.mean(self.gpu_utils),
            "max_utilization": np.max(self.gpu_utils),
            "avg_memory": np.mean(self.gpu_mems),
            "max_memory": np.max(self.gpu_mems),
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x: Tensor, ve: Tensor | None, block_mask: BlockMask):
        ...
        y = flex_attention(q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2), block_mask=block_mask,
                           scale=15 / self.head_dim).transpose(1, 2)
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, num_heads: int, model_dim: int, max_seq_len: int):
        ...
        self.blocks = nn.ModuleList(
            [Block(model_dim, num_heads, max_seq_len, i, num_layers) for i in range(num_layers)])
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        # ve = [value_embed(input_seq) for value_embed in self.value_embeds] # gets 3 different embeddings for the input sequence
        # 012 ... 012 structure on token value embeddings by @YouJiacheng, improved on @leloykun's U-net structure
        # ve = [ve[0], ve[1], ve[2]] + [None] * (len(self.blocks) - 6) + [ve[0], ve[1], ve[2]] # [embeddings, none*6, embeddings] dim = 8
        ...
        for i in range(len(self.blocks)):
            # Pass the list containing x0 and outputs from blocks 0 to i-1
            # Block 'i' will receive a list of length 'i+1'
            x = self.blocks[i](x, None, x0, block_masks[i], prev_outputs)  # Pass ve=None for now

            # Append the output of block 'i' for the next iteration
...
class Hyperparameters:
    # data
    train_files = "/content/drive/MyDrive/fineweb10B/fineweb_train_*.bin"  # input .bin to train on
    val_files = "/content/drive/MyDrive/fineweb10B/fineweb_val_*.bin"  # input .bin to eval validation loss on
