...
class GPT(nn.Module):
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor, sliding_window_num_blocks: Tensor):
        ...
        for i in range(len(self.blocks)):
            ...
            if prev_layers:
                x = self.residual_weights[i][0]*prev_layers[0] + self.x0_weight*x0  # Get weights for layer i
        ...
        for i in range(len(self.blocks)):
            skip_value = None
            if i in skip_map:
                skip_value = self.skip_weights[skip_map[i]] * skip_connections[skip_map[i]]
                x = x + skip_value
...
class Hyperparameters:
    ...
    train_seq_len = 78*1024 # FlexAttention sequence length
...
optimizer2 = Muon(hidden_matrix_params, lr=0.035, momentum=0.95, rank=rank, world_size=world_size)
